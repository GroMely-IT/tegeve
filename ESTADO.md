...
- ⏳ **PENDIENTE (tú):** crear la clave gratis en build.nvidia.com (empieza por `nvapi-`) y guardarla como secreto en los DOS workers: `npx wrangler secret put NVIDIA_API_KEY` (raíz) y `npx wrangler secret put NVIDIA_API_KEY --config worker/wrangler.toml`. No requiere redeploy; el secreto se aplica al instante.

...
- ✅ **Enfoque "recepcionista/consultora" (decisión del dueño): natural, breve y FIABLE por encima de exhaustivo.** Probamos un KB exhaustivo (~10,5 KB): cubría todo pero a ese tamaño Groq se satura en ráfaga (la 2.ª-3.ª pregunta seguida cae al respaldo lento) y el 70B a veces parafrasea de más. Se cambió a: (a) **`TEVI_KB` ligero ~4,2 KB** con lo esencial (empresa, cifras, servicios con sus tecnologías, equipo, sectores, clientes, un caso insignia + ruta a /casos/, historia, reconocimientos, contacto, secciones) → cada petición pesa poco y Groq responde **0,5–0,8 s siempre, sin caídas**; (b) **system prompt de recepcionista** (ES+EN, ambos workers): 2-3 frases, primera persona del plural, cálido; si no tiene un dato fino, **deriva con elegancia** (escríbenos a info@tegeve.es / míralo en /casos/) en vez de "no tengo información". Verificado en vivo. **Para añadir un dato a lo que Tevi sabe → `TEVI_KB` en app.js (mantenerlo compacto).**

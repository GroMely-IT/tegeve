...
- ✅ **Clave NVIDIA puesta en `tegeve` (unificado).** Tevi PROBADO EN VIVO con NVIDIA: explica "¿qué es SAP?" y enlaza a /servicios/sap/, da el teléfono de España (+34 952 569 582) y mantiene el hilo (pregunta de seguimiento sobre oficinas). Producción principal (`tegeve.gabrielgrosso.workers.dev`) al 100%.
- ⏳ **PENDIENTE (tú):** poner la MISMA clave en el worker `tegeve-asistente` (el de GitHub Pages), que aún la tiene vacía. **Desde la raíz del repo**: `npx wrangler secret put NVIDIA_API_KEY --config worker/wrangler.toml`. (Tu intento falló por ejecutarlo dentro de `worker/`, que duplicaba la ruta.)

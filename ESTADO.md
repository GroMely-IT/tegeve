...
- ⚠️ **PENDIENTE de decisión (dominio):** la web vive en **`tegevem.es`**, pero los `canonical`, `og:url`, `sitemap.xml` y el correo apuntan a **`tegeve.es`** (sin "m"). Hay que decidir cuál es el dominio definitivo y alinear todo (canonicals/OG/sitemap/CNAME). Si el bueno es `tegevem.es`, hay que reemplazar `tegeve.es` → `tegevem.es` en esas URLs (el correo `info@tegeve.es` se mantiene si ese buzón sigue en tegeve.es).
- ⚠️ **Latencia de NVIDIA gratis:** picos frecuentes (vistos 32 s y 44 s) por la cola compartida del endpoint de prueba. Recomendado para velocidad CONSTANTE (~2 s): **Groq** (gratis, API compatible con OpenAI) como proveedor primario, con NVIDIA de respaldo. Pendiente de decisión del dueño.

...
- ✅ **KB canónico EXHAUSTIVO (`TEVI_KB`, ~10,5 KB, 15 secciones).** Verificado el "punto dulce": Groq responde <1 s hasta ~10-12 KB de contexto; el sitio entero (55 KB) lo rompe (excede el límite por minuto → respaldo lento). Se sintetizó (workflow, 9 páginas en paralelo) un KB compacto pero completo que cubre: empresa, **cifras**, equipo, los 4 servicios **con tecnologías**, **los 12 casos con sus métricas reales**, sectores, clientes, modelos+método, valores, historia/hitos, reconocimientos, FAQ, contacto por país y mapa de secciones. Antes (KB "lean" ~2,5 KB) faltaba casi todo el detalle de casos/servicios/sectores/FAQ. Historial enviado reducido (cliente -6; workers -6 y 600 chars/turno) para que las conversaciones sigan rápidas. **Para añadir un dato nuevo a lo que Tevi sabe → actualizar `TEVI_KB` en app.js.**

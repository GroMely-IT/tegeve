...
const cio = new IntersectionObserver((entries) => {
  entries.forEach(e => {
    if (!e.isIntersecting) return;
    const el = e.target, target = +el.dataset.count, suffix = el.dataset.suffix || '', prefix = el.dataset.prefix || '';
    let cur = 0; const step = Math.max(1, Math.ceil(target/40));
    const tick = () => { cur = Math.min(target, cur+step); el.textContent = prefix+cur+suffix; if (cur<target) requestAnimationFrame(tick); };
...
// La IA la sirve el Worker unificado "tegeve" (mismo que el sitio, con la clave
// de NVIDIA). Cuando el sitio lo sirve el PROPIO Worker (workers.dev o dominio
// propio como tegevem.es), se llama a /api/tevi en el MISMO origen (sin CORS).
// Solo en GitHub Pages y en localhost (donde no hay Worker) se llama al Worker
// de forma absoluta y cruzada (esos orígenes sí están permitidos por CORS).
var _aiHost = (typeof location !== 'undefined' && location.hostname) || '';
var _aiNoWorker = /(^|\.)github\.io$/.test(_aiHost) || _aiHost === 'localhost' || _aiHost === '127.0.0.1';
const AI_ENDPOINT = _aiNoWorker ? 'https://tegeve.gabrielgrosso.workers.dev/api/tevi' : '/api/tevi';
...
var TEVI_KB = `
# TeGeVe — quiénes somos
TeGeVe (también TGV) es una consultora tecnológica de servicios de software con más de 30 años de trayectoria. Eslogan: "Transformamos los proyectos tecnológicos más desafiantes en soluciones innovadoras". Trabajamos desde España (Málaga), Argentina (Buenos Aires), México y Estados Unidos (Florida), con proyectos en más de 16 países, en modalidad nearshore o en las oficinas del cliente. Web: tegeve.es. Asistente del sitio: Tevi.

# Cifras
Más de 300 profesionales en el equipo. Más de 80 clientes en 16 países; entre ellos, 6 de las 10 compañías de alimentos más grandes del mundo y 3 de las instituciones financieras líderes del mundo. Más de 30 años de trayectoria. CMMI Nivel 3 (DEV).

# Servicios
Cuatro especialidades: (1) SAP: implementación, evolución y soporte, y el camino a SAP S/4HANA (BTP, Fiori, HANA, ABAP, CPI, SAP Analytics Cloud), con soporte AMS. (2) Oracle JD Edwards (EnterpriseOne y World): implantación, upgrades y soporte (Orchestrator, BI Publisher, One View, E1 Pages, JDE Mobile). (3) IA Empresarial y Business Intelligence: BI, IA y automatización (RPA) llevadas a producción —agentes, asistentes, automatización y analítica— sobre Power Platform, OpenAI, Google Cloud, UiPath o SAP Joule. (4) Desarrollo de software a medida: apps web y móviles, integración de sistemas y modernización de legacy (Angular, React, .NET, Java, Node.js; COBOL, AS/400, DB2). Además: Assessment (evaluación y auditoría) y desarrollo para la industria financiera.

# Cómo trabajamos
Modelos: implementación a medida, AMS (soporte evolutivo con SLA), Software Factory, Testing Factory, Staff Augmentation, Assessment y nearshore. Empezamos por entender el negocio con un assessment honesto y acompañamos hasta producción y después. El precio depende del alcance y se define tras un primer diagnóstico, sin compromiso. Para empezar basta una conversación.

# Equipo
Dirección: Osvaldo Tessio, Ernesto Galindez y Marta Vicena (cofundadores) y Gabriel Grosso (Director). Responsables de área: Fernando García (SAP), Julieta Vegas (Oracle ERPs), Fernando Baztarrica (Web Business Solutions), Jose Jaliff (IA Empresarial), Jorge Bessone (Desarrollo para Servicios Financieros), María Amelia Rojas (RR. HH.), José Luis Cárcamo (Calidad y Procesos), Mariano Attanasio (Administración y Finanzas), Carlos Rasch (Ventas y Marketing) y Gustavo Palmieri (IT). Equipo senior, estable y multidisciplinar.

# Sectores y clientes
Trabajamos en banca y finanzas, energía y oil & gas, infraestructuras, retail y distribución, sector público, industria, gran consumo, agro, medios de pago, seguros y logística. Algunos clientes: Motta Internacional, Weatherford, Abertis, Banco Itaú, Banco Comafi, Kimberly-Clark, Nutrien y First Data.

# Casos de éxito
Tenemos casos reales con resultados medibles en banca, energía, agro, gran consumo, seguros y medios de pago; por ejemplo, una conciliación de fondos de inversión con IA que pasó de cuatro días a unas horas. El detalle de cada caso, con sus cifras, está en /casos/, donde se puede filtrar por servicio o sector.

# Historia
Fundada en 1992 en Argentina por Osvaldo Tessio, Ernesto Galindez y Marta Vicena. Hitos: 2004 primer Service Partner de SAP; 2014 oficina en Florida (EE. UU.); 2021 llegada a Málaga (España); 2025 nivel de madurez 3 en el CMMI DEV. Treinta y cuatro años y cuatro países.

# Reconocimientos
CMMI Nivel 3, firmantes del Pacto Global de la ONU, miembros fundadores de Polo IT Buenos Aires y de la comisión directiva de CESSI. Partners de SAP, Oracle e IBM.
...
Servicios: /servicios/ (y el detalle en /servicios/sap/, /servicios/oracle-jd-edwards/, /servicios/ia-empresarial/, /servicios/desarrollo-a-medida/, /servicios/modernizacion-legacy/ y /servicios/assessment/). Historia, equipo y reconocimientos: /nosotros/. Casos de éxito, sectores y clientes: /casos/. Contacto y oficinas: /contacto/.
`;
...
function setScrollTarget(url, terms){
  try {
    if(!terms || !terms.length) return;
    var p = new URL(url, location.href).pathname.replace(/index\.html$/,'');
    sessionStorage.setItem('tevi_scroll', JSON.stringify({ path:p, terms:terms, ts:Date.now() }));
  } catch(e){}
}
// Busca en la página el elemento que mejor coincide con los términos, lo centra
// en pantalla y lo resalta un instante. Devuelve true si lo encontró.
function scrollToTerms(terms){
  if(!terms || !terms.length) return false;
  var T = terms.map(function(t){ return norm(t); }).filter(function(t){ return t.length>2; });
  if(!T.length) return false;
  var root = document.querySelector('main') || document.body;
  var els = root.querySelectorAll('h1,h2,h3,h4,h5,p,li,td,figcaption,span,strong,b,a,div.ct,div.case-client');
  var best=null, bestScore=0;
  els.forEach(function(el){
    var txt = norm(el.textContent||'');
    if(txt.length<2 || txt.length>360) return;
    var score=0; T.forEach(function(t){ if(txt.indexOf(t)>-1) score++; });
    if(score>bestScore){ bestScore=score; best=el; }
  });
  if(best && bestScore>0){
    // Desplazamos la VENTANA con la posición medida (scrollIntoView falla por los
    // transforms de las animaciones .reveal en los ancestros). Centramos el
    // elemento dejando holgura para el nav fijo.
    var rect = best.getBoundingClientRect();
    var y = rect.top + window.pageYOffset - Math.max(96, (window.innerHeight - rect.height) / 2);
    try { window.scrollTo({ top: Math.max(0, y), behavior:'smooth' }); } catch(e){ window.scrollTo(0, Math.max(0, y)); }
    best.classList.add('ai-spot');
    setTimeout(function(){ best.classList.remove('ai-spot'); }, 2600);
    return true;
  }
  return false;
}
...
// Si pulsas un enlace dentro de una respuesta de Tevi, guarda el objetivo de
// scroll (términos de la última pregunta) para enfocar el contenido al llegar.
if(aiBody) aiBody.addEventListener('click', function(e){
  var a = e.target.closest ? e.target.closest('a[href]') : null;
  if(!a) return;
  var href = a.getAttribute('href') || '';
  if(/^(mailto:|tel:|javascript:|#)/i.test(href)) return;
  try { if(new URL(a.href, location.href).origin === location.origin) setScrollTarget(a.href, termsOf(lastQ)); } catch(_e){}
});
// Al cargar la página, si hay un objetivo de scroll pendiente para ESTA página,
// desplaza hasta el contenido relevante y lo resalta.
(function teviScrollOnLoad(){
  var raw; try { raw = sessionStorage.getItem('tevi_scroll'); } catch(e){ return; }
  if(!raw) return;
  try { sessionStorage.removeItem('tevi_scroll'); } catch(e){}
  var t; try { t = JSON.parse(raw); } catch(e){ return; }
  if(!t || !t.path || !t.terms) return;
  if(t.ts && (Date.now() - t.ts) > 30000) return;
  var here = location.pathname.replace(/index\.html$/,'');
  if(here !== t.path) return;
  setTimeout(function(){ scrollToTerms(t.terms); }, 550);
})();
...
(function(){
  ...
  var LANGS6 = ['es','en','pt','it','fr','de'];
  var LANG_LABELS = {es:'ES', en:'EN', pt:'PT-BR', it:'IT', fr:'FR', de:'DE'};
  var LANG_LOCALE = {es:'es_ES', en:'en_US', pt:'pt_BR', it:'it_IT', fr:'fr_FR', de:'de_DE'};
  window.__I18N = window.__I18N || {};
  function dictFor(lang){ if(lang==='en') return DICT; return window.__I18N[lang] || null; }
  function tr(raw, lang){ if(lang==='es') return raw; var t=raw.trim(); var d=dictFor(lang); if(d && d[t]) return raw.replace(t, function(){ return d[t]; }); return raw; }
  ...
  function ensureLang(lang, cb){
    if(lang==='es' || lang==='en' || window.__I18N[lang]){ return cb(); }
    if(_i18nLoading[lang]){ _i18nLoading[lang].push(cb); return; }
    _i18nLoading[lang] = [cb];
    var sc=document.createElement('script');
    sc.src = SITE_ROOT + 'assets/i18n-' + lang + '.js?v=45';
    sc.onload = sc.onerror = function(){ var cbs=_i18nLoading[lang]||[]; _i18nLoading[lang]=null; cbs.forEach(function(f){ try{f();}catch(e){} }); };
  ...
  function applyLangNow(lang){
    ...
    if(m){ document.title=m.title; setName('description',m.desc); setName('twitter:title',m.twt); setName('twitter:description',m.twd); setProp('og:title',m.ogt); setProp('og:description',m.ogd); setProp('og:locale', LANG_LOCALE[lang]||'es_ES'); }
    var curT=document.querySelector('.lang-cur-t'); if(curT) curT.textContent = LANG_LABELS[lang] || lang.toUpperCase();
    ...
    document.querySelectorAll('.lang-switch').forEach(function(sw){ sw.classList.remove('open'); var c=sw.querySelector('.lang-cur'); if(c)c.setAttribute('aria-expanded','false'); });
    try{ localStorage.setItem('tgvLang', lang); }catch(e){}
    document.dispatchEvent(new CustomEvent('langchange',{detail:{lang:lang}}));
  }
  function applyLang(lang){ if(LANGS6.indexOf(lang)<0) lang='es'; ensureLang(lang, function(){ applyLangNow(lang); }); }
  collect();
  document.addEventListener('click', function(e){
    var t=e.target, cur=(t.closest?t.closest('.lang-cur'):null);
    if(cur){ e.preventDefault(); var sw=cur.closest('.lang-switch'); var open=sw.classList.toggle('open'); cur.setAttribute('aria-expanded', open?'true':'false'); return; }
    var opt=(t.closest?t.closest('.lang-opt'):null);
    if(opt){ e.preventDefault(); applyLang(opt.getAttribute('data-lang')); return; }
    document.querySelectorAll('.lang-switch.open').forEach(function(sw){ sw.classList.remove('open'); var c=sw.querySelector('.lang-cur'); if(c)c.setAttribute('aria-expanded','false'); });
  });
